...
${taskList.length > 0 ? taskList.join('\n') : 'No tasks scheduled for today'}`;
  ...
  const sendEODReport = async () => {
    ...
    const allTasksWithStatus = tasks.map((task, index) => {
      ...
      const description = task.description ? `\n   Description: ${task.description}` : '';
      return `${index + 1}. ${task.title} (${statusMap[task.status]})${description}`;
    });
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <div className={`flex-1 transition-all duration-300 ${sidebarCollapsed ? 'lg:ml-16' : 'lg:ml-64'}`}>
        ...
        <main className="p-6">
          ...
          {tasks.filter(t => t.status === 'in_progress' || t.status === 'blocked').length > 0 && (
            <div className="mb-8">
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <h2 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                  ...
                  Unfinished Tasks - Continue When Needed
                </h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {tasks.filter(t => t.status === 'in_progress' || t.status === 'blocked').map(task => (
                    <TaskCard
                      key={task.id}
                      task={task}
