...
import { Task, User } from '@/types/attendance';
...
export default function Dashboard() {
  ...
  // Start with empty tasks - user adds their own
  ...
  const timeIn = () => {
    ...
    // Auto-update all tasks to "in-progress" when timing in
  ...
  // Generate Start Report and copy to clipboard
  const copyStartReport = async () => {
    ...
    const report = `🌊 *GoWater Start Report*
...
Have a productive day! 💪`;
    ...
    try {
      ...
      alert('📋 Start report copied to clipboard! You can now paste it in WhatsApp.');
    } catch (error) {
      console.error('Failed to copy to clipboard:', error);
      // Fallback: show the report in a dialog for manual copying
      prompt('Copy this start report to WhatsApp:', report);
    }
  };

  // Send EOD Report to WhatsApp (end of day)
  const sendEODReport = async () => {
    if (!isWhatsAppConnected) {
      alert('Please connect to WhatsApp first to send reports.');
      return;
    }
    ...
    // Create task list with status and emojis
    const allTasksWithStatus = tasks.map((task, index) => {
      ...
      return `${index + 1}. ${task.title} (${statusMap[task.status]})`;
    });
...
Have a great evening! 🌅`;
    ...
    try {
      ...
      const savedRecipients = localStorage.getItem('whatsapp-recipients');
      const recipients = savedRecipients ? JSON.parse(savedRecipients) : [];
      
      if (recipients.length === 0) {
        alert('No WhatsApp recipients selected. Please configure recipients in the WhatsApp settings.');
        return;
      }

      const result = await whatsappClientService.sendBulkMessage(recipients, report, 'eod-report');
      ...
      if (results.successful.length > 0) {
        alert(`✅ EOD report sent successfully to ${results.successful.length} recipient(s)`);
      }
      
      if (results.failed.length > 0) {
        console.error('Failed to send to some recipients:', results.failed);
        alert(`⚠️ Failed to send to ${results.failed.length} recipient(s). Check console for details.`);
      }
    } catch (error) {
      console.error('Error sending EOD report:', error);
      alert('❌ Failed to send EOD report. Please check your WhatsApp connection.');
    }
  };
      ...
      <div className={`flex-1 transition-all duration-300 ${sidebarCollapsed ? 'lg:ml-16' : 'lg:ml-64'}`}>
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center space-x-2">
                ...
                <button
                  ...
                  title={tasks.length === 0 ? 'Add tasks first before timing in' : 'Start your work day'}
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="flex items-center space-x-2">
                {isTimedIn && (
                  <button
                    ...
                    title={isWhatsAppConnected ? "Send start of work report" : "Connect WhatsApp first"}
                  >
                ...
                <button
                  ...
                  title={isWhatsAppConnected ? "Send end of day report" : "Connect WhatsApp first"}
                >
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0020.885 3.700"/>
                  </svg>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-blue-600">{tasks?.filter(t => t.status === 'in-progress')?.length || 0}</p>
              </div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">Work Duration</p>
